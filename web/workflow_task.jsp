					...
					<c:forEach var="task" items="${taskList}">
	                    <tr>
	                        ...
	                        <td>
	                        	<c:choose>
	                        		<c:when test="${type eq 'leave'}">
	                        			<a href="${pageContext.request.contextPath}/viewTaskForm?taskId=${task.id}&flag=1" class="btn btn-success btn-xs"><span class="glyphicon glyphicon-plus"></span> 办理任务</a>
	                        		</c:when>
	                        		<c:otherwise>
	                        			<a href="${pageContext.request.contextPath}/viewTaskForm?taskId=${task.id}&flag=2" class="btn btn-success btn-xs"><span class="glyphicon glyphicon-plus"></span> 办理任务</a>
	                        		</c:otherwise>
	                        	</c:choose>
